...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDayIndex: number) {
    if (weekDays.includes(weekDayIndex)) {
      const weekDaysWithoutSelectedDay = weekDays.filter(
        (day) => day !== weekDayIndex
      );

      setWeekDays(weekDaysWithoutSelectedDay);
    } else {
      const weekDaysWithSelectDays = [...weekDays, weekDayIndex];

      setWeekDays(weekDaysWithSelectDays);
    }
  }
